...
public class ImageMath {
	...
	/**
	 * Apply a bias to a number in the unit interval, moving numbers towards 0 or 1 according to the
	 * bias parameter.
	 * 
	 * @param a
	 *            the number to bias
	...
	public static float gain(float a, float b) {
		 ...
		 * 
		 * if (a < .001) return 0.0f; else if (a > .999) return 1.0f; if (a < 0.5) return
		 * (float)Math.pow(2 * a, p) / 2; else return 1.0f - (float)Math.pow(2 * (1. - a), p) / 2;
		 */
	...
	/**
	 * A smoothed pulse function. A cubic function is used to smooth the step between two
	 * thresholds.
	 * 
	 * @param a1
	...
	public static void resample(int[] source, int[] dest, int length, int offset, int stride, float[] out) {
		...
		while (i < length) {
			...
			if (inSegment < outSegment) {
				...
				dest[destIndex] = ((int) Math.min(aSum / sizfac, 255) << 24) | ((int) Math.min(rSum / sizfac, 255) << 16) | ((int) Math.min(gSum / sizfac, 255) << 8)
